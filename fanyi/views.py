	...
	try:
		...
		if running_pid:
			for item in running_pid:
				...
				os.kill(int(item['runningPID']), signal.SIGUSR1)
...
def start_monitor_ip(request):
	...
	try:
		...
		for run_id in running_case_id:
			child = subprocess.Popen(['/usr/local/bin/python', '/search/odin/daemon/pyonsg/utils/monitor.py', str(run_id['id'])], shell=False)
